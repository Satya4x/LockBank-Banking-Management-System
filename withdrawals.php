...
$withdrawals_query = "SELECT t.*, u.full_name, u.balance FROM transactions t 
...
<head>
    ...
    <title>Withdrawals - LOCKBANK</title>
...
<body>
    ...
    <div class="content">
        <h2 class="toph2"><i class="fa-solid fa-money-bill-wave"></i> Withdraw Cash</h2>

        <form action="cash_withdrawal.php" method="POST" class="cash-withdrawal-form">
            ...
            <datalist id="accounts">
                <?php while ($user = mysqli_fetch_assoc($users_result)) { ?>
                    <option value="<?php echo $user['account_number']; ?>">
                        <?php echo $user['full_name']; ?> (A/C: <?php echo $user['account_number']; ?>) - Balance: ₹<?php echo number_format($user['balance'], 2); ?>
                    </option>
                <?php } ?>
            </datalist>
            ...
            <button type="submit" class="btn-withdraw"><i class="fa-solid fa-hand-holding-dollar"></i> Withdraw Cash</button>
        </form>

        <h2 class="toph2"><i class="fa-solid fa-clock-rotate-left"></i> Recent Withdrawals</h2>
            ...
            <tbody>
                <?php while ($withdrawal = mysqli_fetch_assoc($withdrawals_result)) { ?>
                    <tr>
                        ...
                        <td><?php echo $withdrawal['full_name']; ?></td>
                        <td>₹<?php echo number_format($withdrawal['amount'], 2); ?></td>
                        <td>₹<?php echo number_format($withdrawal['balance'], 2); ?></td>
                        <td class="<?php echo $withdrawal['status']; ?>">
                            <i class="fa-solid fa-circle <?php echo ($withdrawal['status'] == 'success') ? 'text-success' : (($withdrawal['status'] == 'pending') ? 'text-warning' : 'text-danger'); ?>"></i>
                            <?php echo ucfirst($withdrawal['status']); ?>
                        </td>
                        <td><?php echo date("d M Y, H:i A", strtotime($withdrawal['timestamp'])); ?></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
